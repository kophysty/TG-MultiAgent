...
- `memory_suggestions`:
    ...
    - unique для антиспама: `(chat_id, kind, status, hash(candidate))` или хранить `candidate_hash`
...
- `work_context_cache`:
    - `key text primary key` (например `tasks_15d`, `ideas_recent`, `social_window`)
    - `payload jsonb`
    - `payload_hash text`
...
- Перед записью `chat_messages.text` прогонять через редактирование секретов:
- минимум: `redactTelegramToken` + удаление строк, похожих на `NOTION_TOKEN`/`OPENAI_API_KEY`
...
- При фразах типа "запомни" preference сохраняется и виден в Notion Preferences DB.
- При неявных предпочтениях бот предлагает сохранить (не спамит).
- Work ctx подмешивается только в обсуждениях (auto), CRUD не замедляется.
- Journal не влияет на рабочий контекст без явного запроса.