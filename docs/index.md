# Документация (repo)

В `docs/` мы храним:

- `docs/roadmap/` - план и целевая архитектура (то, к чему идем).
- `docs/current/` - фактически реализованный функционал (то, что уже работает).

Если в репозитории и Notion есть расхождение - сначала правим Notion, затем обновляем документацию.

## Первоисточник (Notion)

- [Base structure (Notion)](https://www.notion.so/web3-future/Base-structure-2d1535c900f08016af17ca1d92c5c9de?t=2d3535c900f0805e884300a99586f20f)
- [Tasks Base MultiAgent (Notion DB, CRUD тесты)](https://www.notion.so/web3-future/2d6535c900f08191a624d325f66dbe7c?v=2d6535c900f08188b3cf000c5b8f5039)

## Быстрые ссылки

- [Текущее состояние (current)](./current/index.md)
- [План (roadmap)](./roadmap/index.md)
- [Execution history](../execution_history/index.md)

# Документация проекта

## Первоисточник

Первоисточник архитектуры и решений - страница Notion:

- [Base structure (Notion)](https://www.notion.so/web3-future/Base-structure-2d1535c900f08016af17ca1d92c5c9de?t=2d3535c900f0805e884300a99586f20f)
- [Tasks Base MultiAgent (Notion DB, CRUD тесты)](https://www.notion.so/web3-future/2d6535c900f08191a624d325f66dbe7c?v=2d6535c900f08188b3cf000c5b8f5039)

Репозиторий содержит структурированную выжимку и рабочие заметки по этой странице. Если возникает расхождение - считаем Notion первичным, затем обновляем `docs/`.

## Оглавление

- [Execution history](../execution_history/index.md)
- [Стек и деплой](./stack-and-deploy.md)
- [N8N (осторожная интеграция)](./n8n.md)
- [Разделение обязанностей](./responsibilities.md)
- [Структура репозитория](./repo-structure.md)
- [AI-оркестрация (Agent Loop)](./agent-loop.md)
- [Memory и предпочтения](./memory.md)
- [Postgres: что хранить и консистентность](./postgres.md)
- [Tool Registry](./tool-registry.md)
- [Notion и MCP](./notion-mcp.md)
- [Выбор языка: Python vs Node/TS](./language-choice.md)
- [Text и Voice пайплайны](./pipelines.md)
- [Статусы в чате](./chat-statuses.md)
- [Документы, индексация, поиск, цитаты](./documents-and-search.md)
- [Будущие интерфейсы](./future-ui.md)
- [Финальный сборочный лист](./final-checklist.md)


