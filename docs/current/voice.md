...
- "Voice: скачиваю..." (эмоджи: ⬇️)
- "Voice: конвертирую (ffmpeg)..." (эмоджи: 🔄)
- "Voice: распознаю (STT)..." (эмоджи: 🎤)
- "Voice: формирую задачу..." / "Voice: планирую..." (эмоджи: 🤔)
- "Voice: выполняю..." (эмоджи: ⚙️)
- финально заменяет текст на: "Распознано: <одна строка...>" (эмоджи: ✅)

Если редактирование статуса не удалось (например, удалили сообщение) - бот продолжит выполнение без падения.

## Отмена

На статус сообщении во время стадий скачивания, конвертации и распознавания есть inline кнопка "Отмена".

- При нажатии бот пытается отменить текущую обработку (abort download, ffmpeg, STT) и удаляет статус сообщение.
- Ничего не пишет в чат.
- Если успеть нажать до вызова STT и AI, токены не тратятся.

Важно: при смене статусов бот переиспользует то же сообщение. Чтобы inline клавиатура не исчезала при `editMessageText`, мы каждый раз передаем `reply_markup` и тем самым сохраняем кнопку до стадии анализа.
