...
### Явная команда "запомни/в память" (заметки памяти)

Если пользователь явно просит "запомни" или "добавь в память", но extractor не смог выделить структурированные preferences, бот все равно предлагает сохранить "сырой факт" как заметку:

- Создается кандидат с `category=memory_note`
- `value_human` - это весь текст после триггера (например после "запомни")
- Сохранение всегда через подтверждение Да/Нет (никаких автосохранений)
- Если пользователь написал только "запомни" без текста, бот задает уточнение: что именно сохранить.
