...
const { registerAdminCommands } = require('./todo_bot_admin');
...
async function registerTodoBot({
  ...
  const adminApi = registerAdminCommands({
    bot,
    chatSecurity,
    pgPool,
    preferencesRepo,
  ...
  const maybeHandleAdminChatMemoryNaturalLanguage = async ({ chatId, text } = {}) => {
    if (!adminApi?.maybeHandleAdminChatMemoryNaturalLanguage) return false;
    return await adminApi.maybeHandleAdminChatMemoryNaturalLanguage({ chatId, text, callChatCompletions });
  };
  ...
  // Admin commands are registered in core/dialogs/todo_bot_admin.js
