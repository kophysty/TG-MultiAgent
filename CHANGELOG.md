...
## [0.2.16] - 2026-01-15
### Added
- Tasks update: после обновления задачи бот теперь показывает детальный summary (как после создания): база, категория, срок, приоритет, статус, ссылка.

### Fixed
- Tasks title: если `title` длиннее 120 символов, бот сокращает заголовок и переносит полный текст в описание, чтобы карточки в Notion не разъезжались.
